...
while true; do
    ...
    if [ "$user_input" == "exit" ]; then
        ...
        if [ ! -z "${search_history["$user_input"]}" ]; then
            # Entry already exists in search history, so skip the search
            echo "You have already searched for $user_input in this session."
        else
            # Add the user input to the search history
            search_history["$user_input"]="Review your search"
-
            if [[ "$user_input" == "*" ]]; then
                # User entered a wildcard, so list all countries
                matching_countries=($(awk -F',' '{print $1}' countries.csv))
                    ...
                    while true; do
                        ...
                        case $option in
                            1)
                                ...
                                search_history["$user_input"]+=" | Capital City: $(echo $country_info | cut -d',' -f2)"
                                ;;
                            2)
                                echo "Currency: $(echo $country_info | cut -d',' -f3)"
                                search_history["$user_input"]+=" | Currency: $(echo $country_info | cut -d',' -f3)"
                                ;;
                            3)
                                echo "Official Language: $(echo $country_info | cut -d',' -f4)"
                                search_history["$user_input"]+=" | Official Language: $(echo $country_info | cut -d',' -f4)"
                                ;;
                            4)
                                echo "Head of Government: $(echo $country_info | cut -d',' -f5)"
                                search_history["$user_input"]+=" | Head of Government: $(echo $country_info | cut -d',' -f5)"
                                ;;
                            5)
